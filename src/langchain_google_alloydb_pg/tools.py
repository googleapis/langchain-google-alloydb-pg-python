...
class AlloyDBSentimentTool(BaseTool):
    ...
    def _run(
        ...
        return str(self.engine._run_as_sync(self._arun(content)))
...
class AlloyDBIfInput(BaseModel):
    ...
    condition: str = Field(
        description="The semantic condition to evaluate (e.g. 'Is the text positive?')"
    )
...
class AlloyDBIfTool(BaseTool):
    ...
    def _run(
        ...
        return bool(self.engine._run_as_sync(self._arun(condition)))
