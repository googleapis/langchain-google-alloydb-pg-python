...
from langchain_google_alloydb_pg import AlloyDBLoader, AlloyDBDocumentSaver, AlloyDBEngine, Column
...
class TestAlloyDBLoaderAsync:
    ...
    async def test_load_from_query_default_sync(self, engine):
        try:
            await self._cleanup_table(engine)
            query = f"""
                    CREATE TABLE IF NOT EXISTS {table_name} (
            ...
            documents = await asyncio.wait([asyncio.new_event_loop().run_in_executor(None, loader.load())])
